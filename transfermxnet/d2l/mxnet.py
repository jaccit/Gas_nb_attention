...
def train_s2s_ch9(model, data_iter, lr, num_epochs, ctx):
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        for batch in data_iter:
            ...
            # print('X.SHAPE',X.shape)          ##X.SHAPE==(64,10)
            # print('Y.SHAPE',Y.shape)          ##Y.SHAPE==(64,10)
            Y_input, Y_label, Y_vlen = Y[:, :-1], Y[:, 1:], Y_vlen-1
            with autograd.record():
                Y_hat, _ = model(X, Y_input, X_vlen, Y_vlen)
                ##########create###########
                # print("Y_hat",Y_hat.shape)                   ##Y_hat.shape==(64,9,180)
                # print("Y_input",Y_input.shape)              ##Y_input.shape==(64,9)
                # print("Y_label",Y_label.shape)              ##Y_label.shape==(64,9)
                # print("X_vlen",X_vlen.shape)                ##X_vlen.shape==(64,)
                # print("Y_vlen",Y_vlen.shape)                ##Y_vlen.shape==(64,)
                # exit()
                ##########create###########
...
def train_GAS_ch9(model, data_utils,batch_size, lr, num_epochs, ctx):
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        for i, (X, Y) in enumerate(data_iter):
            # print(X,X.shape)
            # exit()
            X, Y = nd.array(X).reshape(128,-1).as_np_ndarray(), nd.array(Y).as_np_ndarray()
            # print(X.shape,Y.shape)               ##X=(128,100,16),Y=(128,5)
            vlinz=nd.ones(X.shape[1],).as_np_ndarray()
            with autograd.record():
                dec_output, _ = model(X, Y, vlinz, vlinz)
###################
    ...
    #             l_class = loss1(cl_res, Y[:, :3], cl_weight).sum()
    #             l_conc= loss2(score_res, Y[:, 3:], conc_weight).sum()
    #             n = Y.shape[0]
